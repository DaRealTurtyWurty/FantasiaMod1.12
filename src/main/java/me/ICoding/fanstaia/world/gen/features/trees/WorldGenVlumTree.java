...
public class WorldGenVlumTree extends WorldGenAbstractTree {
	public static final IBlockState LOG = BlockInit.VLUM_LOG.getDefaultState();
	public static final IBlockState LEAF = BlockInit.VLUM_LEAVES.getDefaultState();
	...
	List<WorldGenVlumTree.FoliageCoordinates> foliageCoords;

	public WorldGenVlumTree() {
		super(false);
	}
	...
	void generateLeafNodeList() {
		...
		this.foliageCoords = Lists.<WorldGenVlumTree.FoliageCoordinates>newArrayList();
		this.foliageCoords.add(new WorldGenVlumTree.FoliageCoordinates(this.basePos.up(k), j));
				...
				for (int l = 0; l < i; ++l) {
					...
					if (this.checkBlockLine(blockpos, blockpos1) == -1) {
						...
						if (this.checkBlockLine(blockpos2, blockpos) == -1) {
							this.foliageCoords
									.add(new WorldGenVlumTree.FoliageCoordinates(blockpos, blockpos2.getY()));
						}
					}
				}
			}
	...
	void limb(BlockPos p_175937_1_, BlockPos p_175937_2_, Block p_175937_3_) {
		...
		for (int j = 0; j <= i; ++j) {
			...
			BlockVlumLog.EnumAxis blocklog$enumaxis = this.getLogAxis(p_175937_1_, blockpos1);
			this.setBlockAndNotifyAdequately(this.world, blockpos1,
					p_175937_3_.getDefaultState().withProperty(BlockVlumLog.LOG_AXIS, blocklog$enumaxis));
		}
	}
	...
	private BlockLog.EnumAxis getLogAxis(BlockPos p_175938_1_, BlockPos p_175938_2_) {
		BlockVlumLog.EnumAxis blocklog$enumaxis = BlockVlumLog.EnumAxis.Y;
		...
		if (k > 0) {
			if (i == k) {
				blocklog$enumaxis = BlockVlumLog.EnumAxis.X;
			} else if (j == k) {
				blocklog$enumaxis = BlockVlumLog.EnumAxis.Z;
			}
		}
	 ...
	 */
	void generateLeaves() {
		for (WorldGenVlumTree.FoliageCoordinates WorldGenVlumTree$foliagecoordinates : this.foliageCoords) {
			this.generateLeafNode(WorldGenVlumTree$foliagecoordinates);
		}
	}
	...
	void generateLeafNodeBases() {
		for (WorldGenVlumTree.FoliageCoordinates WorldGenVlumTree$foliagecoordinates : this.foliageCoords) {
			int i = WorldGenVlumTree$foliagecoordinates.getBranchBase();
			BlockPos blockpos = new BlockPos(this.basePos.getX(), i, this.basePos.getZ());

			if (!blockpos.equals(WorldGenVlumTree$foliagecoordinates)
					&& this.leafNodeNeedsBase(i - this.basePos.getY())) {
				this.limb(blockpos, WorldGenVlumTree$foliagecoordinates, LOG.getBlock());
			}
		}
	}
	...
	private boolean validTreeLocation() {
		...
		boolean isSoil = state.getBlock().canSustainPlant(state, this.world, down, net.minecraft.util.EnumFacing.UP,
				((BlockVlumSapling) BlockInit.VLUM_SAPLING));
